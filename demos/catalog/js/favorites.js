function save_favorites(){
    $.ajax({
        url: "fastcode.biz/hpccjs/save_favorites.php",
        type: "POST",
        data: {
            json: json_str
        }
...
function load_favorites(){
    $.ajax({
        url: "fastcode.biz/hpccjs/load_favorites.php",
        type: "POST",
        data: {
            json: json_str
        }
...
function v1_to_v2_layout(json_str){
    ...
    let ret = [];
    _rects.forEach(rect=>{
        ret.push(get_score_arr(rect,_rects));
    });
...
function compat_score(_a,_b){
    let patterns = {
        ...
        "L_R":{points: 100, callback: function(a,b){return a[0] === b[0] + b[2]}},
        "T_B":{points: 100, callback: function(a,b){return a[0] === b[0] + b[2]}}
    };
...
function sort_rects(rects){
    return [
        ...
        return str.split(splitter).map(n=>$.trim(n.split(splitter2)[0]));
    });
}
function $favorite_thumb(fav_str){
    let _sp = fav_str.split('.');
    let package = fav_str.split('hpccjs.')[1].split('.')[0];
    let widget = fav_str.split('hpccjs.')[1].split('.')[0];
    let package = fav_str.split('hpccjs.')[1].split('.')[0];
    let header_text = get_thumb_header_text(package, widget, data);
    console.info(`header_text`,header_text);
}