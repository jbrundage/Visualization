...
function teViewSaves(method){
    ...
    var html = bsModal(method === 'getThemes' ? 'Saved Themes' : 'Saved Serials',body);
    
    $('body').append(html);
    $('#teModal button.close,#teModal .modal-footer > button').click(function(){
        $('#teModal').remove();
    });
}
function bsModal(header,body){
    return '<div id="teModal" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false" style="display: block;">'+
    '<div class="modal-dialog">'+
      '<div class="modal-content">'+
        '<div class="modal-header">'+
          '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>'+
          '<h4 class="modal-title" id="myModalLabel">'+header+'<a class="anchorjs-link" href="#myModalLabel"><span class="anchorjs-icon"></span></a></h4>'+
        '</div>'+
        '<div class="modal-body" style="height:700px;overflow:auto;">'+body+'</div>'+
