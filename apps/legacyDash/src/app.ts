...
export class App extends DockPanel {
    ...
    private _ddlv2XXX = new JSONEditor();
    ...
    parseParams() {
        if (this._url.param("Wuid")) {
            ...
            result.fetchRows().then(async (response: object[]) => {
                ...
                this._ddlv2XXX.json(upgrade(ddl));
    ...
    enter(domNode: HTMLElement, element: d3SelectionType) {
        ...
        this.addWidget(this._ddlv2XXX, "DDL v2", "tab-after", this._dashboard);
        this.addWidget(this._dashboard, "Dashboard", "tab-after", this._ddlv2XXX);
