...
(function (root, factory) {
    ...
    function CanvasGraph() {
        ...
        this.tick_count = 0;
        this.prev_tick_count = 0;
        
        this.extra_log = [];
        this.demo_layout_idx = 0;
//        setInterval(function(){
//            var layouts_arr = Object.keys(CanvasGraphLayouts.layouts);
//            if("undefined" === typeof layouts_arr[g_w.demo_layout_idx]){
//                g_w.demo_layout_idx = 0;
//            }
//            g_w.layout(layouts_arr[g_w.demo_layout_idx]).render();
//            g_w.demo_layout_idx++;
//        },1000);
    ...
    CanvasGraph.prototype.restart_forces = function(){
        this.prev_tick_count = this.tick_count;
    ...
    CanvasGraph.prototype.clear_node_velocity = function() {
...
//    this.layout("CircleHierarchy");
    ...
    this.secondary_label_stroke('rgba(51, 51, 51, 1)');
...
//    }
    ...
    CanvasGraph.prototype.zoom_to_fit = function(_duration) {
        if (!window.currently_translating) {
            ...
            var edge_points = this.find_edge_nodes(this.data().nodes,_zoom_by_force);
    ...
    CanvasGraph.prototype.find_edge_nodes = function(nodes,zoom_by_force) {
...
//        if(this.tick_count > 500 && this.layout() === "Circle"){
//            debugger;
//        }
        ...
        this.data().info.nothing_is_moving = true;
        this.ctx.beginPath();
...
//        }
    ...
    CanvasGraph.prototype.debugTextDump = function(){
        var context = this;
        ...
        var text_arr = [];
        text_arr.push("calls to tick: "+this.tick_count);
...
//            text_arr.push("this.data().nodes["+i+"].fx: "+node.fx);
//            text_arr.push("this.data().nodes["+i+"].fy: "+node.fy);
//            text_arr.push("this.data().nodes["+i+"].dx: "+node.dx);
//            text_arr.push("this.data().nodes["+i+"].dy: "+node.dy);
            text_arr.push("this.data().nodes["+i+"].force_x: "+node.force_x);
            text_arr.push("this.data().nodes["+i+"].force_y: "+node.force_y);
        });
...
//            })
        ...
        this.simulation
                ...
                .force("y", this.forceY)
                .on("tick", function(){
                    if(!context.data().info.nothing_is_moving || context.tick_count <= 500 + context.prev_tick_count){
                        context.tick.call(context);
                    } else {
                        //console.info("context.data().info.nothing_is_moving:", context.data().info.nothing_is_moving);
                    }
                });
...
//        }
    ...
    CanvasGraph.prototype.dragstarted = function(context) {
        ...
        d3v4.event.subject.forEach(function (_subject) {
            _subject.x = context.transform.invertX(_subject.x);
            _subject.y = context.transform.invertY(_subject.y);
